...
model_location = os.path.join(BASE_DIR, "DS", "project_model", "best_cb.pickle")
...
def _filtration_from_json(data):
    ...
    X_data.columns = ['sku', 'items_cnt', 'a', 'b', 'c', 'goods_wght', 'cargotype']
    ...
    filtered_data = pd.DataFrame(columns=['sku', 'items_cnt', 'a', 'b', 'c', 'goods_wght', 'cargotype', 'class_0','class_1','class_2'])
    ml_data = pd.DataFrame(columns=['sku', 'items_cnt', 'a', 'b', 'c', 'goods_wght', 'cargotype'])

    # Выполняем фильтрацию по карготипу, заполняя созданные датафреймы по заданным условиям
    for sku in X_data.index:
        if '340' in X_data.loc[sku, 'cargotype'] or '360' in X_data.loc[sku, 'cargotype']:
            filtered_data = pd.concat([filtered_data, pd.DataFrame([X_data.loc[sku, :]])], ignore_index=True)
            filtered_data['class_0'] = 1
        else:
            ml_data = pd.concat([ml_data, pd.DataFrame([X_data.loc[sku, :]])], ignore_index=True)

    # Заполняем образовавшиеся пропуски нулями и добавляем столбец с заказом (нужен для модели оптимизации)
...
def _filtration_from_df(data):
    ...
    # Создаём датафреймы для отфильтрованых по карготипу данных и для ML
    filtered_data = pd.DataFrame(columns=['orderkey', 'sku', 'items_cnt', 'a', 'b', 'c', 'goods_wght', 'cargotype', 'class_0','class_1','class_2'])
    ml_data = pd.DataFrame(columns=['orderkey', 'sku', 'items_cnt', 'a', 'b', 'c', 'goods_wght', 'cargotype'])
...
def _preprocessing_ml_data(ml_data, cat_columns=['1010', '1011', '110', '120', '130', '1300', '140', '160', '20',
    ...
    # удаляем столбец с заказом, формируем столбец с количеством заказов и приводим столбцы к нужному для модели порядку
    X = X.drop(['orderkey'], axis=1)
    X['sku_cnt'] = X['sku'].nunique()
    X = X.reindex(columns=['sku','goods_wght', 'items_cnt', 'sku_cnt', 'a', 'b', 'c', 'cargotype'])

    # создаём датафрейм со столбцами для категориальных фичей и заполним их нулями
